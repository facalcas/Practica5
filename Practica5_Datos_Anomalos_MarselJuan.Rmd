...
#Hay dos valores sumamente grandes (73 y 49) de Sepal.Lenght en comparación al resto de valores. Y de esta variable también un registro de 0.
#Lo mismo pasa con Sepal.Width dos valores más grandes que el resto (30 y 29), y esta variable tiene un registro negativo y otro de 0.
#Petal.Lenght tiene 3 decimales, a diferencia de las otras variables, y 3 valores más grandes que el resto (63, 23 y 14), además de 3 registros de 0 o muy cercanos.
#Petal.Width tiene un valor de Infinito positivo, y valores también muy cerca de 0. 
```
...
dirty_iris<-dirty_iris%>%
  mutate(Sepal.Length=ifelse(is.finite(dirty_iris$Sepal.Length),yes=dirty_iris$Sepal.Length,no=NA))%>%
  mutate(Sepal.Length=ifelse(dirty_iris$Sepal.Length<=0,yes=NA,no=dirty_iris$Sepal.Length))%>%
  mutate(Sepal.Width=ifelse(is.finite(dirty_iris$Sepal.Width),yes = dirty_iris$Sepal.Width,no=NA))%>%
  mutate(Sepal.Width=ifelse(dirty_iris$Sepal.Width<=0,yes=NA,no=dirty_iris$Sepal.Width))%>%
  mutate(Petal.Length=ifelse(is.finite(dirty_iris$Petal.Length),yes=dirty_iris$Petal.Length,no=NA))%>%
  mutate(Petal.Length=ifelse(dirty_iris$Petal.Length<=0,yes=NA,no=dirty_iris$Petal.Length))%>%
  mutate(Petal.Width=ifelse(is.finite(dirty_iris$Petal.Width),yes=dirty_iris$Petal.Width,no=NA))%>%
  mutate(Petal.Width=ifelse(dirty_iris$Petal.Width<=0,yes=NA,no=dirty_iris$Petal.Width))
...
Obs_Compl_SepLen<-sum(complete.cases(dirty_iris$Sepal.Length))
Porcentaje_Obs_Compl_SepLen<-Obs_Compl_SepLen/length(dirty_iris$Sepal.Length)
Obs_Compl_SepWid<-sum(complete.cases(dirty_iris$Sepal.Width))
Porcentaje_Obs_Compl_SepWid<-Obs_Compl_SepWid/length(dirty_iris$Sepal.Width)
Obs_Compl_PetLen<-sum(complete.cases(dirty_iris$Petal.Length))
Porcentaje_Obs_Compl_PetLen<-Obs_Compl_PetLen/length(dirty_iris$Petal.Length)
Obs_Compl_PetWid<-sum(complete.cases(dirty_iris$Petal.Width))
Porcentaje_Obs_Compl_PetWid<-Obs_Compl_PetWid/length(dirty_iris$Petal.Width)
...
clean_iris%>%
  summarise(Media=mean(Petal.Length),
            ...
            MADM=mad(Petal.Length))

dirty_iris%>%
  summarise(Media=mean(Petal.Length,na.rm=T),
            DesvEst=sd(Petal.Length,na.rm=T),
            Mediana=median(Petal.Length,na.rm=T),
            IQR=IQR(Petal.Length,na.rm=T),
            MADM=mad(Petal.Length,na.rm=T))  
```
...
regla3sigma<- function(x){
  ...
  desvstd<-sd(x,na.rm=T)
  upLim<-media+3*desvstd
  lowLim<-media-3*desvstd
  
  outlier_3sigma<-ifelse(is.na(x),NA,x<lowLim|x>upLim)
  return(outlier_3sigma)
}

# Identificador Hampel
identi_hampel<- function(x,k=3){
  mediana<-median(x,na.rm = T)
  mad_h<-mad(x,constant=1.4826,na.rm = T)
  
  outlier_hampel<-ifelse(is.na(x),NA,abs(x-mediana)/mad_h > k)
  return(outlier_hampel)
}

# Regla boxplot
regla_boxplot<-function(x,k=1.5){
  Q1<-quantile(x,0.25,na.rm=T)
  Q3<-quantile(x,0.75,na.rm=T)
  IQR_val<-IQR(x,na.rm=T)
  
  lowLim<-Q1-k*IQR_val
  upLim<-Q3+k*IQR_val
  
  outlier_boxplot<-ifelse(is.na(x),NA,x<lowLim|x>upLim)
  return(outlier_boxplot)
}
regla_boxplot(clean_iris$Sepal.Length)

# Percentiles
regla_percentiles<-function(x,p_low=0.05,p_high=0.95){
  q_low<-quantile(x,p_low,na.rm=T)
  q_high<-quantile(x,p_high,na.rm=T)
  
  outlier_percentiles<-ifelse(is.na(x),NA,x<q_low|x>q_high)
...
clean_iris<-clean_iris%>%
  mutate(tressigmaPL=regla3sigma(Petal.Length))%>%
  mutate(hampelPL=identi_hampel(Petal.Length))%>%
  mutate(boxplotPL=regla_boxplot(Petal.Length))%>%
  mutate(percentilesPL=regla_percentiles(Petal.Length))
```
...
clean_iris%>%
  summarise(across(c(tressigmaPL,hampelPL,boxplotPL,percentilesPL),sum))

clean_iris%>%
  filter(tressigmaPL==T|hampelPL==T|boxplotPL==T|percentilesPL==T)%>%
...
clean_iris%>%
  pivot_longer(names_to = "Metodo",values_to = "Outlier",tressigmaPL:percentilesPL)%>%
  ggplot(data=.)+geom_point(mapping = aes(x = Metodo,y = Petal.Length,colour = Outlier),position = "jitter")
```
...
dirty_iris<-dirty_iris%>%
  mutate(Petal.Length1=ifelse(is.na(Petal.Length),mean(Petal.Length,na.rm=T),Petal.Length))
```
...
dirty_iris<-dirty_iris%>%
  group_by(Species)%>%
  mutate(Petal.Length2=ifelse(is.na(Petal.Length),mean(Petal.Length,na.rm=T),Petal.Length))
dirty_iris

dirty_iris%>%
  group_by(Species)%>%
  summarise(promedio=mean(Petal.Length,na.rm=T))
```
...
dirty_iris%>%
  mutate(tressigmaPL=regla3sigma(Petal.Length2))%>%
  mutate(hampelPL=identi_hampel(Petal.Length2))%>%
  mutate(boxplotPL=regla_boxplot(Petal.Length2))%>%
  mutate(percentilesPL=regla_percentiles(Petal.Length2))%>%
  select(Petal.Length,Petal.Length1,Petal.Length2,tressigmaPL:percentilesPL)
...
dirty_iris%>%
  select(Petal.Length,Petal.Length1,Petal.Length2,Species)%>%
  mutate(PL1_outlier=regla_percentiles(Petal.Length1),
         PL2_outlier=regla_percentiles(Petal.Length2))%>%
  filter(PL1_outlier!=1&PL2_outlier!=1)%>%
  mutate(Imputado=ifelse(is.na(Petal.Length),T,F))%>%
  ggplot(data=.)+geom_point(mapping = aes(x = Petal.Length1,y = Petal.Length2, colour=Imputado),position = "jitter")
